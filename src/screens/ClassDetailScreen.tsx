...
// 出席内訳 横棒グラフ
const ATT_BAR_ITEMS = [
  { key: 'present',   label: '出席', color: '#3eb370', mono: false },
      ...
      <SafeAreaView edges={['top']} style={s.headerSafeArea}>
        <View style={s.customHeader}>
          ...
          <TouchableOpacity
            ...
            style={s.backButton}
          >
            <Ionicons name="ellipsis-horizontal" size={22} color="#3eb370" />
          </TouchableOpacity>
        </View>
      </SafeAreaView>
        ...
        {TABS.map(tab => (
          ...
          >
            <Ionicons
              name={tab.icon}
              size={16}
              color={activeTab === tab.key ? '#3eb370' : '#8E8E93'}
            />
          ...
          <View style={s.asgSummaryCard}>
            <View style={s.asgSummaryRow}>
              <View style={s.asgSummaryBlock}>
                <Text style={[s.asgSummaryNum, { color: '#3eb370' }]}>{asgStats.submitted}</Text>
              ...
              <>
                ...
                <View style={s.asgProgressRow}>
                  <View style={s.asgProgressTrack}>
                    <View style={[s.asgProgressFill, { width: `${asgStats.rate}%` as any, backgroundColor: '#3eb370' }]} />
                  </View>
                  <Text style={[s.asgProgressLabel, { color: '#3eb370' }]}>{asgStats.rate}%</Text>
                </View>
              </>
            )}
          </View>
              ...
              <View style={s.card}>
                {pending.length === 0 ? (
                  ...
                  pending.map((item, idx) => {
                    ...
                    const dueColor = isOverdue ? '#FF3B30' : isToday ? '#3eb370' : '#8E8E93';
                    return (
            ...
            >
              ...
              {newDueEnabled && showAsgCalendar && (
                <Calendar
                  ...
                  markedDates={{
                    [newDue]: { selected: true, selectedColor: '#3eb370' },
                  }}
                  theme={{
                    todayTextColor: '#3eb370',
                    arrowColor: '#3eb370',
              ...
              {newDueEnabled && (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <Ionicons name="notifications-outline" size={18} color={newNotify ? '#3eb370' : '#8E8E93'} />
            ...
            <ScrollView contentContainerStyle={s.editScroll} showsVerticalScrollIndicator={false}>
              ...
              <View style={s.editCard}>
                <View style={s.editRow}>
                  ...
                  <TextInput style={s.editTextInput} value={editName.replace(/ /g, ' ')} onChangeText={t => setEditName(t.replace(/ /g, ' '))} placeholder="例：微分積分学" placeholderTextColor="#C7C7CC" autoCorrect={false} />
                </View>
                <View style={s.editDivider} />
                <View style={s.editRow}>
                  <Text style={s.editRowLabel}>教員名</Text>
                  <TextInput style={s.editTextInput} value={editTeacher.replace(/ /g, ' ')} onChangeText={t => setEditTeacher(t.replace(/ /g, ' '))} placeholder="例：山田 太郎" placeholderTextColor="#C7C7CC" autoCorrect={false} />
                </View>
                <View style={s.editDivider} />
                <View style={s.editRow}>
                  <Text style={s.editRowLabel}>教室</Text>
                  <TextInput style={s.editTextInput} value={editRoom.replace(/ /g, ' ')} onChangeText={t => setEditRoom(t.replace(/ /g, ' '))} placeholder="例：A棟 201号室" placeholderTextColor="#C7C7CC" autoCorrect={false} />
                </View>
              </View>
              ...
              >
                ...
                {showCalendar && (
                  <Calendar
                    ...
                    markedDates={{
                      [attDate]: { selected: true, selectedColor: '#3eb370' },
                    }}
                    theme={{
                      todayTextColor: '#3eb370',
                      arrowColor: '#3eb370',
...
const s = StyleSheet.create({
  ...
  tabItemActive: { borderBottomColor: '#3eb370' },
  tabLabel: { fontSize: 13, color: '#8E8E93', fontWeight: '500' },
  tabLabelActive: { color: '#3eb370', fontWeight: '600' },
  ...
  addBtnText: { fontSize: 13, color: '#3eb370', fontWeight: '600' },
  ...
  checkboxDone: { backgroundColor: '#3eb370', borderColor: '#3eb370' },
  asgInfo: { flex: 1 },
  ...
  editHeaderCancel: { fontSize: 16, color: '#3eb370' },
  editHeaderTitle: { fontSize: 17, fontWeight: '600', color: '#1C1C1E' },
  editHeaderSave: { fontSize: 16, fontWeight: '600', color: '#3eb370' },
  ...
  editSegItemActive: { backgroundColor: '#3eb370' },
